...
# Load data from X_file_path csv file and return dask cudf  
def load_X_data(X_file_path):
    
    try:
        # Load data from the CSV file which exclude the first row which is indices and assuem the second row as header in pandas dataframe
        df_X = pd.read_csv(X_file_path, sep=',' , skiprows=[0], header=1 )  
        print ( "X  type ", (type(df_X)))
        
        # df_X = df_X.iloc [1:, :]  
        print ( "X  dataframe shape ", df_X.shape)
        print ( "X  ", df_X) 
...
# Combine X and Y cudf and create dask_cudf out of them
...
def find_y_cluster(cluster_labels):
    ...
    y_cluster = cluster_labels.loc[len(cluster_labels) - 1]
...
# Calculate distances between Y and feature data points in the same cluster
def calculate_distances(combined_data,  cluster_labels, cluster_id):
    ...
    for idx in cluster_indices:
        ...
        distances.append((idx, euclidean(combined_data.iloc[idx].to_numpy(), combined_data.iloc[len(cluster_labels) - 1].to_numpy())))  #.values.flatten()  values.get()
...
def select_closest_features(X_file_path, distances, num_features):
    ...
    print ( "selected_features 3 type ", type(selected_features))
...
def write_to_csv(X_selected, output_path):
    ...
    try:
      ...
      # Create the output directory if it doesn't exist                                                        
       ...
       file_name = 'cuml_dask_cluster_kmeans.csv'
       file_path = os.path.join(output_path, file_name)
                
       table_df.to_csv(file_path, sep = ',', header =True, index = True ) 

       file_path_dict = {'cuml_dask_cluster_kmeans': file_path}
